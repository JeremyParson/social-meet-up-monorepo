...
export default function EventDetails() {
    ...
    // GET SPECIFIED EVENT
    useEffect(() => {
        fetch(`${process.env.REACT_APP_SERVER_URL}/events/${name}`)
            .then(res => res.json())
            .then(data => {
                console.log(data)
                setEventData(data)
    ...
    const deleteData = () => {
        fetch(`${process.env.REACT_APP_SERVER_URL}/events/${name}`, deleteOptions)
            .then(res => res.json())        
    }
    ...
    return (
        <div>
            <header className="my-3">
                <h1 className="text-3xl font-bold text-stone-900">{eventData.event_name}</h1>
            </header>
